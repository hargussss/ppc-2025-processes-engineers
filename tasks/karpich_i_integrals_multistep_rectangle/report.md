     ...
     * Для каждой строки i от start_i до end_i:
       - Для всех j от 0 до divisions-1:
         - Для всех k от 0 до divisions-1:
           - Вычисляются координаты: x = (i + 0.5) * h, y = (j + 0.5) * h, z = (k + 0.5) * h
...
Производительность измерялась на 1, 5, 10, 15 и 20 процессах. Для каждого количества процессов выполнялись два типа измерений:
...
1. **Ускорение для task_run**: Максимальное ускорение достигается при 20 процессах, что является лучшим результатом среди всех конфигураций. При 10 процессах также наблюдается хорошее ускорение. Однако при 15 процессах ускорение незначительно снижается, это может быть связано с неравномерностью распределения нагрузки.

2. **Ускорение для task_pipeline**: Максимальное ускорение достигается при 5 процессах, после чего наблюдается снижение эффективности. При 20 процессах ускорение падает, что указывает на значительные накладные расходы на коммуникацию и синхронизацию.

3. **Эффективность**: Для task_run эффективность постепенно снижается с увеличением количества процессов, что является предсказуемым поведением. При 5 процессах эффективность составляет 68.2%, что является хорошим показателем. Для task_pipeline эффективность снижается сильнее, особенно заметно при большом количестве процессов.
