 ...
 protected:
  ...
  bool CheckTestOutputData(OutType &output_data) final {
    for (std::size_t i = 0; i < output_data.size(); i++) {
      if (abs((output_data[i] - check_data_[i])) > task_eps_) {
        return false;
      }
    }
    return true;
...
const std::array<TestType, 2> kTestParam = {std::make_tuple("genarate with 4x seed 666", 4, 666, 0.0001),
                                            std::make_tuple("genarate with 14x seed 666", 14, 666, 0.0001)};

const auto kTestTasksList = std::tuple_cat(
    ppc::util::AddFuncTask<KarpichISeidolMethodMPI, InType>(kTestParam, PPC_SETTINGS_karpich_i_seidel_method),
    ppc::util::AddFuncTask<KarpichISeidolMethodSEQ, InType>(kTestParam, PPC_SETTINGS_karpich_i_seidel_method));
